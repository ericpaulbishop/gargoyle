...
awk -v lng="$target_lang" '{ lines[x++] = $0 } END { for (y=0; y<=x;) { if (match(lines[y],"CONFIG_PACKAGE_plugin-gargoyle-i18n")) { lines[y]="# "lines[y]; } print lines[y++]} print "CONFIG_PACKAGE_plugin-gargoyle-i18n=y"; printf ("CONFIG_PACKAGE_plugin-gargoyle-i18n-%s=y\n\n", lng)}' "$target_dir/.config" >> "$target_dir/.config2"
