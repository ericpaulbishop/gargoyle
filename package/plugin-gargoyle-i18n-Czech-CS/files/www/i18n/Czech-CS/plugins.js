/*
 * UTF-8 (with BOM) Czech-CS text strings for plugins.sh html elements
 */

pgS.mPlugins="Doplňky";
pgS.PgOpt="Možnosti doplňku";
pgS.PRoot="Root doplňku";
pgS.RDrv="Root disk";
pgS.Chroot="Změnit Root doplňku";
pgS.PgSrc="Zdroje doplňku";
pgS.ANam="Přidat název";
pgS.Aurl="Přidat URL";
pgS.APSrc="Přidat zdroj doplňku";
pgS.PList="Seznam doplňků";
pgS.NoPkg="Balíčky nebyly nalezeny. Obnovit seznam doplňků.";
pgS.RfshP="Načíst ze zdrojů doplňky";
pgS.NoWan="Je nutné připojení k internetu pro aktualizaci doplňků.";

//javascript
pgS.NInst="nainstalována";
pgS.PInst="Předinstalované";
pgS.Instd="Instalované";
pgS.Vers="Verze";
pgS.Stus="Stav";
pgS.RDSpc="Požadované místo na disku";
pgS.USBErr="Balíček nelze nainstalovat (Vyžaduje podporu USB)";
pgS.DskErr="Balíček nelze nainstalovat (Nedostatek místa na disku)";
pgS.ChrootWarn="přesměrovávají Váš kořenový adresář doplňků na USB disk. To znamená, že nesmíte tento disk odpojit, aby vaše doplňky pracovaly správně .. \n\nPokračovat?";
pgS.CharErr="CHYBA: Neplatný znak(y) v názvu zdroje";
pgS.SpcErr="(mezery nejsou povoleny)";
pgS.SNamErr="CHYBA: Je třeba zadat název zdroje";
pgS.SURLErr="CHYBA: Je nutné zadat URL adresu zdroje";
pgS.DupSNamErr="CHYBA: Duplicitní název zdroje";
pgS.DupSURLErr="CHYBA: Duplicitní zdrojová URL";
pgS.AddPSErr="Nelze přidat zdroj doplňku";
pgS.Totl="Celkem";
pgS.Free="Volné";
pgS.Prst="Předvolba";
pgS.Name="Název";
pgS.Langs="Jazyky";
pgS.Thems="Témata";
pgS.Pkgs="Balíčky";
pgS.NoSpace="Nedostatek místa pro";
pgS.PlgSz="Nainstalované doplňky";
pgS.FreeSp="Volné místo";
pgS.Rslv="Prosím odinstalujte některé doplňky a zkuste to znovu, \nnebo zvolte jiný cíl.";
