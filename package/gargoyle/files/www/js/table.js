...
function addTableRow(table, rowData, rowsAreRemovable, rowsAreMovable, rowRemoveCallback, rowMoveCallback, controlDocument) {
  ...
  if (rowsAreMovable) {
    ...
    cellContent.className = "btn btn-default";
    cellContent.onclick = function() {
      moveThisCellsRowUp(this);
      rowMoveCallback(this, "up");
    };
    cell = controlDocument.createElement('td');
    cell.className = table.id + '_column_' + cellIndex;
    cell.appendChild(cellContent);
    ...
    cellContent.className = "btn btn-default";
    cellContent.onclick = function() {
      moveThisCellsRowDown(this);
      rowMoveCallback(this, "down");
    };
    cell = controlDocument.createElement('td');
    cell.className = table.id + '_column_' + cellIndex;
    cell.appendChild(cellContent);
...
function moveThisCellsRowUp(button) {
  ...
  rowIndex = 1;
  while (rowIndex < allRows.length && allRows[rowIndex] != row) {
    rowIndex++;
  }
  if (rowIndex > 1) {
...
function setRowClasses(table, enabled) {
  rows = table.rows;
  rows[0].className = enabled == true ? 'header_row' : 'header_row disabled';
  ...
  while (rowIndex < rows.length) {
    if (enabled == true) {
      ...
      rows[rowIndex].className = rowIndex % 2 == 0 ? 'disabled' : 'disabled';
    }
