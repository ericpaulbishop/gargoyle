...
function resetData() {
  ...
  if (bridgeSection == "") {
    ...
    while (wanDnsList.length > 0) {
      //document.getElementById("wan_dns").appendChild( document.createElement("br") );
      document.getElementById("wan_dns").appendChild(document.createTextNode(" " + wanDnsList.shift()));
    ...
    if (wirelessModeId != "disabled") {
      ...
      if (otherSsid == null) {
        ...
        if (currentWirelessMacs.length > 0 && otherIsSta) {
          setChildText("wan_mac", currentWirelessMacs[0]);
        }
      }
