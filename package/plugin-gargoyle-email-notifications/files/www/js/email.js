...
function testMail() {
    ...
    if (data['auth']) {
		...
		command = command + body + 'sendmail '+data['receiver']+' --syslog --timeout 30 && rm /etc/msmtprc && mv /tmp/msmtprc.tmp /etc/msmtprc';
    } else {
		if(!data['encryption']){
