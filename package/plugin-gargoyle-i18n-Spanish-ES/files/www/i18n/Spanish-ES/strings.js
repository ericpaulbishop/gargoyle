/*
 * UTF-8 (with BOM) Spanish-ES text strings for User Interface elements
 */

UI.SaveChanges="Guardar Cambios";
UI.Reset="Restablecer";
UI.Clear="Eliminar Historial";
UI.Delete="Eliminar Datos";
UI.DNow="Descargar Ahora";
UI.Visited="Sitios Visitados";
UI.Requests="Peticiones de Búsqueda";
UI.Add="Agregar";
UI.Remove="Eliminar";
UI.DDNSService="Servicio DDNS";
UI.WakeUp="Despertar";
UI.NewRule="Agregar Nueva Regla";
UI.NewQuota="Agregar Nueva Cuota";
UI.AddRule="Agregar Regla";
UI.AddSvcCls="Agregar Clase de Servicio";
UI.Edit="Editar";
UI.Select="Seleccionar";
UI.ChPRoot="Cambiar Raíz de complementos";
UI.AddPSource="Agregar Origen de complementos";
UI.Uninstall="Desinstalar";
UI.Install="Instalar";
UI.RefreshPlugins="Actualizar complementos";
UI.GetBackup="Hacer Copia de Seguridad Ahora";
UI.RestoreConfig="Restaurar Configuración Ahora";
UI.RestoreDefault="Resetear a valores de fábrica";
UI.Upgrade="Actualizar Ahora";
UI.Reboot="Reiniciar Ahora";
UI.MoreInfo="Más información";
UI.Hide="Ocultar texto";
UI.WaitSettings="Por favor espere mientras se aplica la nueva configuración. . .";
UI.Wait="Espere por favor. . .";
UI.ErrChanges="Los cambios no se pudieron aplicar";
UI.Disabled="Desactivado";
UI.Enabled="Activado";
UI.Always="Siempre";
UI.Sunday="domingo";
UI.Monday="lunes";
UI.Tuesday="martes";
UI.Wednesday="miércoles";
UI.Thursday="jueves";
UI.Friday="viernes";
UI.Saturday="sábado";
UI.Sun="dom";
UI.Mon="lun";
UI.Tue="mar";
UI.Wed="mié";
UI.Thu="jue";
UI.Fri="vie";
UI.Sat="sáb";
UI.unk="desconocido";
UI.HsNm="Nombre de Equipo";
UI.HDsp="Visualización de Equipos";
UI.DspHn="Mostrar Nombres de Equipos";
UI.DspHIP="Mostrar IPs de Equipos";

UI.never="nunca";
UI.disabled="desactivado";
UI.both="ambos";
UI.seconds="segundos";
UI.minutes="minutos";
UI.hours="horas";
UI.days="días";
UI.second="segundo";
UI.minute="minuto";
UI.hour="hora";
UI.day="día";
UI.month="mes";
UI.year="año";
UI.sc="s"; //abbr for second
UI.hr="h"; //abbr for hour
UI.pAM="De la mañana";
UI.pPM="De la tarde";
UI.hAM="a.m.";
UI.hPM="p.m.";

UI.EMonths=["ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"];

UI.CApplyChanges="Cerrar y Aplicar Cambios";
UI.CDiscardChanges="Cerrar y Descartar Cambios";
UI.waitText="Por Favor, Espere Mientras Se Aplican los cambios";
UI.Cancel="Cancelar";

UI.byt="bytes";
UI.Bu="B";
UI.KB="kB";
UI.MB="MB";
UI.GB="GB";
UI.TB="TB";
UI.KB1="kByte";
UI.MB1="MByte";
UI.GB1="GByte";
UI.TB1="TByte";
UI.KBy="kBytes";
UI.MBy="MBytes";
UI.GBy="GBytes";
UI.TBy="TBytes";
UI.Kbs="kbits/s";
UI.KBs="kBytes/s";
UI.MBs="MBytes/s";

UI.Err="ERROR";
UI.prfErr="Existe un error en";
UI.nex="no existe";
UI.InvAdd="ERROR: Dirección Inválida";

UI.CPass="Confirmar Contraseña";
UI.OK="OK";
UI.VPass="Verificando Contraseña...";
UI.sprt="Puerto de SSH";
UI.wsprt="puerto del servidor web";
UI.prdr="puerto redireccionado al enrutador";
UI.puse="puerto en uso por el enrutador";
UI.pfwd="puerto redireccionado a";
UI.conn="conectado";

UI.AJAX="Se requiere un navegador con soporte de AJAX";
UI.AJAXUpg="Por favor actualice a un navegador compatible con AJAX y vuelva a intentarlo. Estos navegadores incluyen Firefox 2.0+, Safari y IE 6+ entre otros.";
