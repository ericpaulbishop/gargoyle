...
xsystem(const char *argv[])
{
	...
	switch (pid) {
	case -1:
		targz_perror(ERROR, "%s: vfork", argv[0]);
		return -1;
	case 0:
		/* child */
	...
	if (waitpid(pid, &status, 0) == -1) {
		targz_perror(ERROR, "%s: waitpid", argv[0]);
		return -1;
	}

	if (WIFSIGNALED(status)) {
		targz_msg(ERROR, "%s: Child killed by signal %d.\n",
	...
	if (!WIFEXITED(status)) {
		/* shouldn't happen */
		targz_msg(ERROR, "%s: Your system is broken: got status %d "
			"from waitpid.\n", argv[0], status);
		return -1;
	}
