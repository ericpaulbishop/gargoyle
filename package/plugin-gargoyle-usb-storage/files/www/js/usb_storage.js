...
function saveChanges() {
    ...
    uci.set("vsftpd", "global", "anonymous_write", haveAnonymousFtp ? "yes" : "no");
    if (ftpWanPasv) {
        uci.set("vsftpd", "global", "pasv_min_port", pasvMin);
        uci.set("vsftpd", "global", "pasv_max_port", pasvMax)
    }
    var postCommands = [];
    if (uciOriginal.get("firewall", ftpFirewallRule, "local_port") != uci.get("firewall", ftpFirewallRule, "local_port") || uciOriginal.get("firewall", pasvFirewallRule, "start_port") != uci.get("firewall", pasvFirewallRule, "start_port") || uciOriginal.get("firewall", pasvFirewallRule, "end_port") != uci.get("firewall", pasvFirewallRule, "end_port")) {
...
function resetData() {
    ...
    if (storageDrives.length > 0) {
        ...
        document.getElementById("ftp_wan_pasv").checked = wanFtp && pmin != "" && pmax != "" || !wanFtp;
        updateWanFtpVisibility();
        userNames = uciOriginal.getAllSectionsOfType("share_users", "user");
        var tableObject;
        if (userNames.length > 0) {
            var userTableData = [];
            var userIndex;
        ...
        var getMounted = function(shareList, config) {
            ...
            for (shareIndex = 0; shareIndex < shareList.length; shareIndex++) {
                ...
                if (shareDrive != null) {
                    ...
                    var shareData = sharePathToShareData[fullSharePath] == null ? ["", "", "", "", "", false, false, false, "none", [],
                        [], "ro", "*"
                    ] : sharePathToShareData[fullSharePath];
                    if (shareData[0] == "" || config == "samba") {
                        shareData[0] = uciOriginal.get(config, shareId, "name");
                        shareData[0] == "" ? shareId : shareData[0]
                    }
                    shareData[1] = shareDrive;
                    shareData[2] = shareMountPoint;
                    shareData[3] = shareDirectory;
                    shareData[4] = fullSharePath;
                    shareData[5] = config == "samba" ? true : shareData[5];
                    shareData[6] = config == "vsftpd" ? true : shareData[6];
                    shareData[7] = config == "nfsd" ? true : shareData[7];
...
function shareSettingsToDefault(controlDocument) {
    ...
    var defaultData = ["share_" + (sharePathList.length + 1), currentDrive, driveToMountPoints[currentDrive][0], "", driveToMountPoints[currentDrive][0], true, true, true, "none", [],
        [], "ro", "*"
    ];
...
function getVisStr(vis) {
    var shareTypeStr = function(type) {
        return vis[type] ? type.toUpperCase() : ""
    };
...
function getShareDataFromDocument(controlDocument, originalName) {
    ...
    if (userAccessTable != null) {
        ...
        for (uatIndex = 0; uatIndex < userAccessTableData.length; uatIndex++) {
            ...
            if (rowData[1] == "R/O") {
                roUsers.push(rowData[0])
            }
            if (rowData[1] == "R/W") {
                rwUsers.push(rowData[0])
            }
        }
    }
...
function setShareTypeVisibility(controlDocument) {
    ...
    var getTypeDisplay = function(type) {
        return vis[type] ? type.toUpperCase() : ""
    };
...
function editShare() {
    ...
    editShareWindow = window.open("usb_storage_edit.sh", "edit", "left=" + xCoor + ",top=" + yCoor);
...
function reboot() {
    ...
    testReboot = function() {
        toggleReload = true;
        setTimeout("testReboot()", 5 * 1e3);
        document.getElementById("reboot_test").src = testLocation
    };
    setTimeout("testReboot()", 25 * 1e3);
    setTimeout("reloadPage()", 240 * 1e3)
}
