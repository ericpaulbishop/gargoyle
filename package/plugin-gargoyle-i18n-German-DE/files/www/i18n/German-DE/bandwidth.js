/*
 * UTF-8 (with BOM) German-DE text strings for bandwidth.sh/bandwidth_distribution.sh html elements
 */

bndwS.mBandwidth="Bandbreitennutzung";
bndwS.GOpSect="Anzeigeoptionen für die genutzte Bandbreite.";
bndwS.TFrm="Zeitrahmen";
bndwS.year="Jahr";
bndwS.Plot="Plot";
bndwS.TBdw="Gesamt";
bndwS.None="keine";
bndwS.HRInf="Speichere Daten aller Rechner mit 15 minütiger Auflösung.";
bndwS.HRWrn="Nicht empfohlen für Router mit weniger RAM als 32MB";
bndwS.UsInf="Gesamte Datennutzung bezieht sich auf die des Internetzugangs.";
bndwS.LclTrff="Datenverkehr zwischen lokalen Rechner wird nicht gemessen.";
bndwS.BGrSect="Darstellung der genutzten Bandbreite.";
bndwS.Dnld="Download";
bndwS.Upld="Upload";
bndwS.Totl="Total";
bndwS.BUTab="Tabelle der übertragenen Datenmenge";
bndwS.DspI="Intervall";
bndwS.qhour="Viertelstunden";
bndwS.mnths="Monate";
bndwS.DspT="Typ";
bndwS.DspID="Display Id";
bndwS.TbUnt="Einheit";
bndwS.AutoM="Automatisch";
bndwS.DelD="Daten löschen";
bndwS.DBData="Daten herunterladen";
bndwS.DSep="Daten sind durch Komma separiert.";
bndwS.DFmt="[Direction],[Interval Länge],[Intervals Saved],[IP],[Interval Start],[Interval End],[Bytes Used]";
bndwS.DNow="Jetzt herunterladen";
bndwS.expd="vergrößern";

//javascript
bndwS.UpCl="Upload Class";
bndwS.DlCl="Download Class";
bndwS.Time="Zeit";
bndwS.BPlot="Bandbreitengrafik Plot";
bndwS.BUsag="Bandbreitennutzung";
bndwS.RstGr="Grafiken zurücksetzen...";
bndwS.DelAD="Lösche alle Daten?";
bndwS.DelDW="Daten werden gelöscht...";

//bandwidth_distribution.sh
bndwS.DOpSect="Anzeigeoptionen der Bandbreitenverteilung";
bndwS.DTFrm="Zeitintervall";
bndwS.DtbI="Verteilung der Bandbreite";
bndwS.BDst="Verteilung der Bandbreite";
bndwS.BDtbl="Verteilung der Bandbreite (Tabelle)";
bndwS.FMonths=["Januar", "Februar", "März", "April", "Mai", "Juni", "Juli", "August", "September", "Oktober", "November", "Dezember"];
bndwS.Down="Down";
bndwS.Up="Up";
bndwS.Sum="Summe";
bndwS.Host="Rechner";
