...
prtS.scnd="s";
prtS.UPHelp="UPnP (ang. Univeral Plug and Play) i NAT-PMP (ang. NAT Port Mapping Protocol) są to dwa protokoły, które pozwalają urządzeniom i aplikacją w sieci LAN na automatyczną konfigurację routera w zakresie przekierowania portów w celu poprawnego działania. Jeżeli urządzenie wspiera jeden z tych protokołów to nie ma potrzeby ręcznego tworzenia reguł przekierowania, ponieważ urządzenie może wykonać je samodzielnie.</p><p>Kiedy jest to włączone, Gargoyle pokazuje tabelę z automatycznie utworzonymi przekierowanami, więc można łatwo sprawdzić czy ta usługa działa poprawnie. Natomiast mogą być z nią problemy, jeżeli konfiguracja sieci zawiera dwa routery (podwójny NAT). Jeżeli tabela zawiera tylko jeden wiersz z '***', to oznacza że nie ma żadnego zarejestrowanego przekierowania.</p><p>Jako część protokołu, urządzenia w sieci LAN mogą żądać prędkości połączenia WAN routera. Dwa pola odpowiadają za konfigurację takich zapytań. Aplikacja kliencka może używać tych informacji do optymalizowania jej wydajności. Należy zauważyć że router nie robi żadnych operacji bazując na tych danych, są one tylko zwracane do urządzenia pytającego. Jeżeli zostało wprowadzone zero jako wartość, zwykle zwracane jest 100MB lub 1GB w zależności od szybkości interfejsu routera.</p><p>There is some controversy about the security of this service and it does require additional RAM to run which may be important on memory constrained routers, so by default this feature is off.</p><p>Istnieją pewne problemy z bezpieczeństwem tych usług oraz wymagają one dodatkowej pamięci RAM do działania, co może mieć znaczenie dla niektórych routerów. Domyślnie są więc wyłączone.";
