/*
 * UTF-8 (with BOM) Polish-PL text strings for overview.sh html elements
 */

ovwS.mDevice="Urządzenie";
ovwS.mWAN="WAN";
ovwS.mLAN="LAN";
ovwS.mWireless="Wi-Fi";
ovwS.mTime="Czas";
ovwS.mServices="Usługi";
ovwS.mBridge="Most";
ovwS.Sts="Status";
ovwS.DNam="Nazwa urządzenia";
ovwS.GVer="Wersja Gargoyle";
ovwS.Modl="Model";
ovwS.DevC="Konfiguracja urządzenia";
ovwS.MemU="Zajętość pamięci RAM";
ovwS.SwMemU="Zajętość pamięci wymiany";
ovwS.Conns="Połączenia";
ovwS.CPUAvg="Średnie obciążenie CPU";
ovwS.Uptm="Czas pracy";
ovwS.CDaT="Bieżąca data i czas";
ovwS.BrIPA="Adres IP Mostu";
ovwS.BrNMsk="Maska podsieci Mostu";
ovwS.BrMAdd="Adres MAC Mostu";
ovwS.LGtwy="Adres IP bramy LAN";
ovwS.Cvia="Połączony przez";
ovwS.BrID="SSID Mostu";
ovwS.LIPA="Adres IP LAN";
ovwS.LNmsk="Maska podsieci LAN";
ovwS.LMAdd="Adres MAC LAN";
ovwS.WIPA="Adres IP WAN";
ovwS.WNmsk="Maska podsieci WAN";
ovwS.WMAdd="Adres MAC WAN";
ovwS.WGtwy="Adres bramy WAN";
ovwS.WDNS="Serwer(y) DNS WAN";
ovwS.WUptm="Czas połączenia PPPoE";
ovwS.W3GSS="Siła sygnału 3G/LTE";
ovwS.WlMod="Tryb pracy Wi-Fi";
ovwS.WlMAdd="Adres MAC bezprzewodowy";
ovwS.APID="Nazwa AP (SSID)";
ovwS.F5GID="Nazwa AP 5GHz (SSID)";
ovwS.IDJoin="Połączony jako klient do SSID";
ovwS.QUp="QoS (wysyłanie)";
ovwS.QDwn="QoS (pobieranie)";
ovwS.TEMPcpu="Temperatura CPU";
ovwS.TEMPmem="Temperatura RAM";
ovwS.TEMPwifi="Temperatura Wi-Fi";

//javascript
ovwS.AcPt="Punkt dostępowy";
ovwS.Clnt="Klient";
ovwS.Gtwy="Brama sieciowa";
ovwS.WBrgR="Most bezprzewodowy/przekaźnik";
ovwS.Discon="Rozłączony";
ovwS.T2p4GID="Nazwa AP 2.4GHz (SSID)";
ovwS.ClBr="Most kliencki";
ovwS.Port="Port";
